...
int semInit( unsigned int nsems )
{
    ...
    // 初始化信号量组中的各个元素的值
    semctl(sem_id , 0 , SETVAL , 0 ); // 设置信号量组中的0号元素的资源量为 0  【可读资源量】
    semctl(sem_id , 1 , SETVAL , 1 ); // 设置信号来组中的1号元素的资源量为 1  【可写资源量】
...
void readMem( int sem_id , shmInfo shm  )
{
    ...
    while(1)
    {
        ...
        // 进入临界区前要申请资源   P操作
        struct sembuf sopsp ={
            .sem_num = 0 ,   // 设置需要操作的信号量数组的元素下标  【可读资源】
            .sem_op = -1  // 设置操作的资源量为 -1  （p 操作 ，申请一个资源） 【申请读取的权限】
        };
        semop(sem_id , &sopsp, 1 );
        printf("等待申请得到读取内存的权限成功，进入共享内存读取数据...\n");
        ...
        // 离开临界区应该释放资源 V操作
        sopsp.sem_num = 1 ;  // 设置需要操作的信号量数组元素的下标为  1 【可写资源】
        sopsp.sem_op = 1 ; // 设置操作资源量为 1  (v 操作 ， 释放一个资源) 【释放了一个可写的资源】
        semop(sem_id , &sopsp, 1 );
        printf("释放一个可写资源...\n");
