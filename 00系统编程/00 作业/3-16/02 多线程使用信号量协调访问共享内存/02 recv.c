...
void * wrietMem( void * cntl  )
{

    memCntl cnl = * (memCntl*)cntl ;
    printf("%d , %d %d\n" , __LINE__ , cnl.sem_id , cnl.shm.shm_id );
    while(1)
    {
        printf("等待申请得到写入内存的权限...\n");
        // 进入临界区前要申请资源   P操作
        struct sembuf sopsp ={
            .sem_num = 2 ,   // 设置需要操作的信号量数组的元素下标  【可写资源】
            .sem_op = -1  // 设置操作的资源量为 -1  （p 操作 ，申请一个资源） 【申请写入的权限】
        };
        semop(cnl.sem_id , &sopsp, 1 );
        printf("等待申请得到写入内存的权限成功，进入共享内存写入数据...\n");


        // 读取共享内存
        fgets(cnl.shm.map+MAP_SEEK_WRITE , 4096 , stdin );


        // 离开临界区应该释放资源 V操作
        sopsp.sem_num = 3 ;  // 设置需要操作的信号量数组元素的下标为  1 【可读资源】
...
void readMem( int sem_id , shmInfo shm)
{
    ...
    while(1)
    {
        ...
        // 进入临界区前要申请资源   P操作
        struct sembuf sopsp ={
            .sem_num = 1 ,   // 设置需要操作的信号量数组的元素下标  【可写资源】
            .sem_op = -1  // 设置操作的资源量为 -1  （p 操作 ，申请一个资源） 【申请写入的权限】
        };
        ...
        // 离开临界区应该释放资源 V操作
        sopsp.sem_num = 0 ;  // 设置需要操作的信号量数组元素的下标为  1 【可读资源】
        sopsp.sem_op = 1 ; // 设置操作资源量为 1  (v 操作 ， 释放一个资源) 【释放了一个可读的资源】
        semop(sem_id , &sopsp, 1 );
        printf("释放一个可读资源...\n");
...
int main(int argc, char const *argv[])
{
    ...
    // 信号量初始化
    cntl.sem_id = semInit( 4 );

    printf("%d , %d %d\n" , __LINE__ , cntl.sem_id , cntl.shm.shm_id );


    pthread_t thread ;
    pthread_create( &thread, NULL , wrietMem , (void *)&cntl);


    readMem(cntl.sem_id , cntl.shm);
